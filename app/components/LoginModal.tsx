...
import { auth, googleProvider, microsoftProvider, appleProvider } from '@/lib/firebase/config';
...
const LoginModal = ({ isOpen, closeModal }: LoginModalProps) => {
  ...
  const [providerLoading, setProviderLoading] = useState<'google' | 'microsoft' | 'apple' | null>(null);
  ...
  const handleProviderLogin = async (provider: 'google' | 'microsoft' | 'apple') => {
    ...
    try {
      ...
      setError(`Error al iniciar con ${provider === 'google' ? 'Google' : provider === 'microsoft' ? 'Microsoft' : 'Apple'}`);
    ...
    >
      ...
      >
        ...
        <div className="space-y-3 mb-4">
          <button 
            type="button"
            className="w-full bg-[#9c7800] text-white py-2 rounded-lg flex justify-center items-center hover:bg-[#b58d00] transition-colors"
          ...
          <button 
            type="button"
            className="w-full bg-[#00A4EF] text-white py-2 rounded-lg flex justify-center items-center hover:bg-[#0088D1] transition-colors"
            onClick={() => handleProviderLogin('microsoft')}
            disabled={!!providerLoading}
          >
            <FaMicrosoft className="mr-2 text-lg" />
            {providerLoading === 'microsoft' ? 'Cargando...' : 'INICIAR CON MICROSOFT'}
          </button>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="w-full bg-[#06e96c] text-white py-2 rounded-[20px] hover:bg-[#85f89e] transition-colors flex justify-center items-center"
            disabled={loading || !!providerLoading}
          >
            {loading ? (
